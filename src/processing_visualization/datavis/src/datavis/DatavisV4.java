...
public class DatavisV4 extends PApplet {
	...
	public static void main(String[] args) {
		PApplet.main(DatavisV4.class.getName());
	}
	...
	public void setup() {
		...
		try {
			...
			ShapeUtil.findFrame(this).setResizable(!config.SHOW_FULL_SCREEN);
			...
			// Shrink the logo by 50%, using zero as the height param to
			...
			// TODO: Finish image carousel:
			// http://www.openprocessing.org/sketch/13104
	...
	public void draw() {
		...
		if (showImageGallery) {
			...
			// HACK: The background() function in Processing has a bug, so set
			// as full-size background image.
			image(currentBackgroundImage, 0, 0);

			// Reset the inactivity counter (to work with
			// "show gallery after hash button click" hack).
			inactivityCounter = 0;
		} else {
	...
	private void saveAndUploadScreenshot() {
		...
		String screenshotName = config.SAVED_IMAGE_DIR + "screenshots" + File.separator + hashtag + "_" + simpleDateFormat.format(startDate) + "_" + simpleDateFormat.format(endDate) + "_" + THEME
				+ SAVED_IMAGE_FORMAT;
	...
	private void checkForDotHover() {
		if (lmController.isConnected()) {
			...
			if (foremost.isValid() && frame.hands().count() == 1) {
				...
				LOGGER.info("(" + pointerX + ", " + pointerY + ")");

				// Draw guide markers.
				ShapeUtil.drawCircle(this, pointerX, pointerY, radiusMin, new int[] { 255, 0, 0 });
			}
		}
		...
		for (Iterator<HeartbeatEvent> i = allHeartbeatEvents.getAllHeartbeatEvents().iterator(); i.hasNext();) {
			...
			if (item.getOuterDot() != null && item.getOuterDot().containsPoint(new Point2D.Float(pointerX, pointerY))) {
	...
	private void displayEventToolip(HeartbeatEvent heartbeatEvent) {
		...
		int textSize = 12;
		try {
			ShapeUtil.drawRectangleWithText(this, center.x + rectOuterPadLeft, center.y - rectOuterPadTop, rectCurve, padLeft, padRight, padTop, padBottom, TOOLTIP_RGB, TEXT_RGB, textSize, lines,
					false, true);
		} catch (Exception ex) {
			lines = new String[] { "[Unable to render the specified text.]" };
			ShapeUtil.drawRectangleWithText(this, center.x + rectOuterPadLeft, center.y - rectOuterPadTop, rectCurve, padLeft, padRight, padTop, padBottom, TOOLTIP_RGB, TEXT_RGB, textSize, lines,
					false, true);
		}
	}
	...
	private void renderEventSet(List<HeartbeatEvent> heartbeatEvents) {
		for (Iterator<HeartbeatEvent> i = heartbeatEvents.iterator(); i.hasNext();) {
			...
			// FIXME: Augment the Dot positions with pointer position...
			/*
			 * The pointer will have an invisible circle. If the dots are in the
			 * circle, their origins are pushed away from the origin of the
			 * pointer, in the direction of a vector from origin to
			 * circomference. (Research particle avoidance algorithms?)
			 * 
			 * UPDATE: Don't do the above. Instead, the pointer circle should
			 * bring up the detail window. Try doing this in 3D, with the
			 * details upfront?
			 */
		...
		for (Iterator<HeartbeatEvent[]> i = dotArcs.iterator(); i.hasNext();) {
			...
			// Find center value between the two event angles (in degrees).
